...
###DM436_2L_10000 data, 10000 means the window-width
...
#read from url:
ratiotable36 = read.table("http://alchemy.duhs.duke.edu/~gd44/Yulong/Yulong_1024/DM436_Bin2RPKM_2L_10000R.txt", sep="\t", col.names=c("chr_id", "start", "reads"))

X11()
par(mfrow = c(3, 1), mar = c(2, 4, 3, 2))

#plot the figure for DM436 chromosome 2L only (BLACK)
plot((ratiotable36$reads), main="DM436 Chr_2L, 10k RPKM ",cex=.3)
...
##############################################
#read table for DM435_2L_10000 data, read from url:
ratiotable35 = read.table("http://alchemy.duhs.duke.edu/~gd44/Yulong/Yulong_1024/DM435_Bin2RPKM_2L_10000R.txt", sep="\t", col.names=c("chr_id", "start", "reads"))


#plot the figure for DM435 chromosome 2L only
plot(ratiotable35$reads, main="DM435 Chr_2L , 10k",cex=.3)
...
#plot then points:
plot(ratiotable36$reads *1000, main="Chr_2L DM436(black) vs DM435(red), 10k RPKM ",cex=.3)

points(ratiotable35$reads*1000, main="Chr_2L DM436(black) vs DM435(red), 10k",cex=.3, col="red")
...
#plot then points:
plot(filter(ratiotable36$reads *1000, rep(1,15)/15), main="Chr_2L DM436(black) vs DM435(red), 10k with filter RPKM ",cex=.3)

points(filter(ratiotable35$reads*1000, rep(1,15)/15), main="Chr_2L DM436(black) vs DM435(red), 10k with filter",cex=.3, col="red")
...
plot(log2(filter(ratiotable36$reads,rep(1,15)/15))-log2(filter(ratiotable35$reads, rep(1,15/15))), 
       main="Chr_2L log2 Ratio:DM436 over DM435, 10",cex=.3)
...
plot((log2(ratiotable36$reads)-log2(ratiotable35$reads)), 
       ylim=c(-5,5), main="log2 Chr_2L Ratio:DM436 over DM435, 10",cex=.3)
