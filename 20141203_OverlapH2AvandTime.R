...
# plot part one, cutoff = 0.5
########################################################
X11()
png(file="H2AvTimingOverlap.png")       ## make a new png figure;

par(mfrow = c(3, 1), mar = c(2, 4, 3, 2))

#plot the H2Av figure for cutoff=0.5
title1 = ( "cutoff 1.0")
plot((ratiotable10$reads),type = "l", main= title1, cex=.3, xlim = c(0, 100))

par(new=T) 
#plot the Timing figure for cutoff=0.5
ratiotableTime0.5 = read.table("http://alchemy.duhs.duke.edu/~gd44/Yulong/Yulong_1203/StretchRegionWithTime_cutoff0.0.txt", sep="\t", col.names=c("index", "reads"))
